		...
		for (int[] pCoord : getPawnsCoord("B"))
			for(int[] eCoord : escapePoints)
				if (kingCoord[0] == eCoord[0] && eCoord[0] == pCoord[0]) {
					if (kingCoord[1] > pCoord[1] && pCoord[1] >= eCoord[1] && isCleanHorizontal(kingCoord[0], eCoord[1],pCoord[1]) && isCleanHorizontal(kingCoord[0],pCoord[1] + 1,kingCoord[1]))
						result++;
					else if(kingCoord[1] < pCoord[1] && pCoord[1] <= eCoord[1] && isCleanHorizontal(kingCoord[0], kingCoord[1] + 1, pCoord[1]) && isCleanHorizontal(kingCoord[0],pCoord[1] + 1,eCoord[1] + 1))
						result++;
				}
				else if (kingCoord[1] == eCoord[1] && eCoord[1] == pCoord[1]) {
					if (kingCoord[0] > pCoord[0] && pCoord[0] >= eCoord[0] && isCleanVertical(kingCoord[1],eCoord[0],pCoord[0]) && isCleanVertical(kingCoord[1],pCoord[0] + 1, kingCoord[0]))
						result++;
					else if(kingCoord[0] < pCoord[0] && pCoord[0] <= eCoord[0] && isCleanVertical(kingCoord[1], kingCoord[0] + 1, pCoord[0]) && isCleanVertical(kingCoord[1],pCoord[0] + 1, eCoord[0] + 1))
						result++;
				}
